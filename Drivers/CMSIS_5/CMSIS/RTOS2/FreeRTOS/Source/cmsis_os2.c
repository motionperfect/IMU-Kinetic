...
osKernelState_t osKernelGetState (void)
{
  ...
  switch (xTaskGetSchedulerState ())
	{
	  case taskSCHEDULER_RUNNING:state = osKernelRunning;
	  break;

	  case taskSCHEDULER_SUSPENDED:state = osKernelLocked;
	  break;
...
int32_t osKernelLock (void)
{
  ...
  if (IS_IRQ())
  ...
  else
	{
	  switch (xTaskGetSchedulerState ())
		{
		  case taskSCHEDULER_SUSPENDED:lock = 1;
		  break;

		  case taskSCHEDULER_RUNNING:vTaskSuspendAll ();
		  lock = 0;
		  break;

		  case taskSCHEDULER_NOT_STARTED:
		  default:lock = (int32_t)osError;
		  break;
		}
	}
	  ...
	  switch (xTaskGetSchedulerState ())
		{
		  ...
		  case taskSCHEDULER_RUNNING:lock = 0;
		  break;
		  ...
		  if (IS_IRQ())
			{
			  if (timeout == 0U)
				{
				  if (xSemaphoreTakeFromISR ((SemaphoreHandle_t)&mp->sem, NULL)
					  == pdTRUE)
					{
					  if ((mp->status & MPOOL_STATUS) == MPOOL_STATUS)
						{
			...
			{
			  if (xSemaphoreTake ((SemaphoreHandle_t)&mp->sem, timeout)
				  == pdTRUE)
				{
				  if ((mp->status & MPOOL_STATUS) == MPOOL_STATUS)
					{
  ...
  else
	{
	  ...
	  if ((mp->status & MPOOL_STATUS) != MPOOL_STATUS)
	  ...
	  else
		{
		  ...
		  if (IS_IRQ())
			{
			  if (uxSemaphoreGetCountFromISR ((SemaphoreHandle_t)&mp->sem)
				  == mp->bl_cnt)
				{
				  stat = osErrorResource;
				}
			...
			{
			  if (uxSemaphoreGetCount ((SemaphoreHandle_t)&mp->sem)
				  == mp->bl_cnt)
				{
				  stat = osErrorResource;
				}
