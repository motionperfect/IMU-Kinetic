  ...
  if ((flags > 0U) && (flags < 0x80000000U))
  ...
  else
	{
	  switch ((int32_t)flags)
		{
		  case osErrorResource:event.status = osOK;
		  break;
		  case osErrorTimeout:event.status = osEventTimeout;
		  break;
		  case osErrorParameter:event.status = osErrorValue;
		  break;
		  default:event.status = (osStatus)flags;
		  break;
		}
	}
  return event;
...
osTimerId
osTimerCreate (const osTimerDef_t *timer_def, os_timer_type type, void *argument)
{
...
int32_t osSemaphoreWait (osSemaphoreId semaphore_id, uint32_t millisec)
{
  ...
  switch (status)
	{
	  case osOK:count = osSemaphoreGetCount (semaphore_id);
	  return ((int32_t)count + 1);
	  case osErrorResource:
	  case osErrorTimeout:return 0;
	  default:break;
	}
  return -1;
}
...
os_InRegs osEvent osMessageGet (osMessageQId queue_id, uint32_t millisec)
{
  ...
  switch (status)
	{
	  case osOK:event.status = osEventMessage;
